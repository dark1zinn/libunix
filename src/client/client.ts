...
export class Client<OutgoingEvents extends Record<string, unknown> = Record<string, unknown>> {
  ...
  async disconnect(): Promise<void> {
    ...
    this.pending.rejectAll(
      new LibunixError("CONNECTION_LOST", "Client disconnected"),
    );
    if (this.socket) {
  ...
  private async establishConnection(): Promise<void> {
    this.socket = await connectWithRetry(
      () => this.openSocket(),
  ...
  private async dispatch(frame: DecodedFrame): Promise<void> {
    ...
    if (envelope.kind === "error") {
      this.pending.reject(
        key,
        new LibunixError(
          normalizeErrorCode(envelope.error.code),
  ...
  private handleConnectionLost(error?: Error): void {
    ...
    const reason =
      error ??
      new LibunixError("CONNECTION_LOST", "Connection to server was lost");
    this.pending.rejectAll(
      isLibunixError(reason)
        ? reason
        : new LibunixError("CONNECTION_LOST", reason.message),
    );
  }
}
