...
function assertEmitCorrelation(
  type: MessageTypeValue,
  correlationId: Uint8Array,
): void {
...
export function encodeFrame(
  ...
  const bodyLength =
    1 + CORRELATION_ID_SIZE + payload.length;
  validateBodyLength(bodyLength);
...
export function decodeFrame(frame: Uint8Array): DecodedFrame {
  ...
  if (frame.length < expectedSize) {
    throw new FrameError(
      `Incomplete frame: have ${frame.length}, need ${expectedSize}`,
    );
  }
  if (frame.length > expectedSize) {
    throw new FrameError(
      `Frame buffer has ${frame.length - expectedSize} trailing bytes`,
    );
  }
  ...
  const correlationId = frame.slice(
    FRAME_CORRELATION_OFFSET,
    FRAME_PAYLOAD_OFFSET,
  );
