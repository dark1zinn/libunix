...
function createSocketHandlers(handlers: TransportSocketHandlers): SocketHandler<BunSocketData> {
    return {
...
export class BunTransportAdapter implements TransportAdapter {
    ...
    async connect(path: string, handlers: TransportSocketHandlers): Promise<TransportSocket> {
