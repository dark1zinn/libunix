...
describe("encodeFrame / decodeFrame", () => {
  ...
  test("EVENT_EMIT uses zero correlation", () => {
    ...
    expect(correlationIdsEqual(decoded.correlationId, zeroCorrelation())).toBe(
      true,
    );
  });
  ...
  test("rejects EVENT_EMIT with non-zero correlation on encode", () => {
    expect(() =>
      encodeFrame(MessageType.EventEmit, correlation(1), new Uint8Array(2)),
    ).toThrow(FrameError);
  });
  ...
  test("rejects incomplete buffer on decode", () => {
    ...
    expect(() => decodeFrame(frame.subarray(0, frame.length - 1))).toThrow(
      FrameError,
    );
  });
