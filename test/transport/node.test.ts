...
import { createNodeTransportAdapter } from '../../src/transport/node.ts';

function tempSocketPath(): string {
    return join(tmpdir(), `libunix-transport-node-${crypto.randomUUID()}.sock`);
}

describe('NodeTransportAdapter', () => {
    const adapter = createNodeTransportAdapter();
